...
class Loss(nn.Module):
    ...
    def forward(self, pred_tensor, target_tensor):
        ...
        coord_mask = coord_mask.unsqueeze(dim=-1).expand_as(target_tensor) # [n_batch, S, S] -> [n_batch, S, S, N]
        noobj_mask = noobj_mask.unsqueeze(dim=-1).expand_as(target_tensor) # [n_batch, S, S] -> [n_batch, S, S, N]
...
#         self.b = num_boxes
#
#     def compute_iou(self, predictions: Tensor, targets: Tensor) -> Tensor:
#         """
#
#         Parameters
#         ----------
...
#
#         # batch size
#         n = predictions.shape[0] / s / s
#
#         x = b * 5 + c
#         boxes_1 = predictions.view(-1, x)  # NxSxS x B*5+C
#         boxes_2 = targets.view(-1, x)
#         # reshape to NxSxS x B*5 by slicing off the last +C
#         x = b * 5
#         boxes_1 = boxes_1[:, :x]
#         boxes_2 = boxes_2[:, :x]
#         # reshape to NxSxSxB x 5
#         x = n * s * s * b
#         boxes_1 = boxes_1.view(x, 5)
#         boxes_2 = boxes_2.view(x, 5)
#         # slice off the 5th element
#         boxes_1 = boxes_1[:, :4]
#         boxes_2 = boxes_2[:, :4]
#
#         # # skip every 5th result
#         # boxes_1 = boxes_1[:, torch.mod(torch.arange(boxes_1.shape[1])) != 0]
#         # boxes_2 = boxes_2[:, torch.mod(torch.arange(boxes_2.shape[1])) != 0]
#         # convert (x, y, w, h) to (x1, y1, x2, y2)
#         boxes_1[:, 2:4] = boxes_1[:, :2] + boxes_1[:, 2:4]
#         boxes_2[:, 2:4] = boxes_2[:, :2] + boxes_2[:, 2:4]
#
#         ious = box_iou(boxes_1, boxes_2)
#
#         return ious
#
#
...
#         ----------
#         predictions:
#             (N, S, S, B*5+C) prediction tensor where the 5 is (x, y, w, h, p_obj)
#         targets:
#             (N, S, S, B*5+C) target tensor where the 5 is (x, y, w, h, p_obj)
#
#         Returns
#         -------
#
...
#         boxes_1 = 9
#
#         iou = self.compute_iou(predictions.view(-1, ), targets)
#
#         return iou
